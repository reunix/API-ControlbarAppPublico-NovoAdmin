...
{
  constructor(
    @InjectRepository(PontosEntregasListRespostaDto)
    private readonly repositorio: Repository<PontosEntregasListRespostaDto>
  ) {}
  ...
  async buscarFornecedoresAdmin(
    ...
    try {
      const produtos: PontosEntregasListRespostaDto = await this.repositorio.query(
        sql,
        [cliente_id]
      );
