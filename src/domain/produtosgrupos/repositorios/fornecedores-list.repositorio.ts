...
{
  constructor(
    @InjectRepository(ProdutosGruposListRespostaDto)
    private readonly repositorio: Repository<ProdutosGruposListRespostaDto>
  ) {}
  ...
  async buscarFornecedoresAdmin(
    ...
    try {
      const produtos: ProdutosGruposListRespostaDto = await this.repositorio.query(
        sql,
        [cliente_id]
      );
