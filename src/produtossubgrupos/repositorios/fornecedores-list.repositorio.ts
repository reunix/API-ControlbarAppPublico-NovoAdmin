...
@Injectable()
export class FornecedorListRepositorioImpl
  implements FornecedorListAdminRepositorio
{
  constructor(
    @InjectRepository(ProdutosSubGruposListRespostaDto)
    private readonly repositorio: Repository<ProdutosSubGruposListRespostaDto>
  ) {}

  async buscarFornecedoresAdmin(
    cliente_id: number
    ...
    try {
      const produtos: ProdutosSubGruposListRespostaDto =
        await this.repositorio.query(sql, [cliente_id]);

      return produtos;
    } catch (error) {
      console.error('Erro ao buscar fornecedores do cliente:', error);
      throw new Error('Erro ao executar a consulta de fornecedores');
    }
  }
}
