...
{
  constructor(
    @InjectRepository(FornecedorListRespostaDto)
    private readonly repositorio: Repository<FornecedorListRespostaDto>
  ) {}
  ...
  async buscarFornecedoresAdmin(
    ...
    try {
      const produtos: FornecedorListRespostaDto = await this.repositorio.query(
        sql,
        [cliente_id]
      );
